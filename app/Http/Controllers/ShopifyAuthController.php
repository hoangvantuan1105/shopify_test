...
class ShopifyAuthController extends Controller
{
    ...
    public function auth(Request $request): RedirectResponse
    {
        ...
        // Kiểm tra định dạng hợp lệ của tên miền myshopify (cho phép chữ cái, chữ số, dấu gạch nối '-' và dấu gạch dưới '_')
        ...
        // Tạo nonce ngẫu nhiên an toàn để chống CSRF
        $state = Str::random(40);

        // Lưu state vào session và cache (hạn 15 phút) để bước callback xác thực
        session([
        ...
        // Xây dựng URL ủy quyền của Shopify
    ...
    /**
     * Xử lý callback sau khi merchant đồng ý cài đặt trên Shopify.
     */
    public function callback(Request $request): JsonResponse
    {
        ...
        // 1. Kiểm tra sự tồn tại của các tham số bắt buộc
        if (!$shop || !$code || !$hmac || !$state) {
            abort(400, 'Thiếu tham số bắt buộc từ Shopify callback (shop, code, hmac, state).');
        }

        // 2. Validate định dạng shop
        if (!preg_match('/^[a-zA-Z0-9][a-zA-Z0-9\-_]*\.myshopify\.com$/i', $shop)) {
            abort(400, 'Tên miền shop không hợp lệ.');
        }

        // 3. Xác thực state để chống CSRF (kiểm tra session hoặc cache)
        ...
        // 4. Xác thực chữ ký HMAC từ Shopify
        ...
        // 5. Gửi request sang Shopify để đổi authorization code lấy access_token vĩnh viễn
